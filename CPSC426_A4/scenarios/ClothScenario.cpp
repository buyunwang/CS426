...
void cClothScenario::BuildCloth()
{
	...
	{
		int v = 0;
		int id = mCloth.GetVertID(u, v, z);
		cCloth::tPin pin;
		pin.mVert = id;
		pin.mPos = mCloth.GetVertPos(mCloth.GetVertID(u, v, z));
		mCloth.AddPin(pin);
	}
	}
}
...
for (int z = 0; z < res[2] - 1; ++z)
{
	for (int v = 0; v < res[1] - 1; ++v)
	{
		...
		{
			int v0 = mCloth.GetVertID(u, v, z);
			int v1 = mCloth.GetVertID(u + 1, v, z );
			int v2 = mCloth.GetVertID(u + 1, v + 1, z);
			int v3 = mCloth.GetVertID(u, v + 1, z);
