...
void cBirdScenario::UpdateCharacter()
{
	...
	Eigen::VectorXd D1;
	mCurve.EvalTangent(curr_time, D1);
	Eigen::Vector3d P1 = Eigen::Vector3d(D1[0], D1[1], D1[2]);
//	std::cout << "T is " << std::endl;
//	std::cout << T << std::endl;
//	
	Eigen::VectorXd D2;
	mCurve.EvalNormal(curr_time, D2);
	Eigen::Vector3d P2 = Eigen::Vector3d(D2[0], D2[1], D2[2]);
//	std::cout << "S is " << std::endl;
//	std::cout << S << std::endl;
	
	Eigen::Vector3d T = P1.normalized();
	Eigen::Vector3d B = P1.cross(P2);
	if (B.norm() < 0.01){
		B = P1.cross(P2 + Eigen::Vector3d(0,0.01,0));
	}
    B.normalize();
...
//    std::cout << B << std::endl;
	...
	mCharTransform.block(0, 0, 3, 1) = T;
	mCharTransform.block(0, 1, 3, 1) = N;
	mCharTransform.block(0, 2, 3, 1) = B;

//	tVector N = B.cross(Tv);
//	std::cout << "N is " << std::endl;
//	std::cout << N << std::endl;
	
//	tVector pos = tVector(N[0], N[1], N[2], 0);
//	mCharTransform.setIdentity();
//	mCharTransform.block(0, 3, 3, 1) = pos.segment(0, 3);
...
//	cDrawUtil::Translate(tVector(0, 0.5, -1, 0));
	cDrawUtil::Translate(tVector(-mTime/10, 0.5, mTime/10, 0));
...
//	cDrawUtil::DrawBox(tVector(0.5, 0.5, -1, 0), tVector(0.2, 0.2, 0.2, 0));
    cDrawUtil::Rotate(tVector(0, mTime/10,mTime/2, 0));
    cDrawUtil::DrawBox(tVector(0.5*mTime/10, -0.5*mTime/10, mTime/100, 0), tVector(0.2, 0.2, 0.2, 0));
	cDrawUtil::Scale(tVector(mTime/2, 1, 1, 0));

	cDrawUtil::PushMatrix();
//	cDrawUtil::Translate(tVector(-0.5, 0.5, -1, 0));
    cDrawUtil::Translate(tVector(-0.5, 0.5*mTime/10, mTime/10, 0));
	SetColor(tVector(0.15, 0.2, 0.85, 1));
	cDrawUtil::DrawCylinder(0.2, 0.05);
	cDrawUtil::PopMatrix();
	
	SetColor(tVector(0.5, 0.2, 0.8, 3));
//	cDrawUtil::DrawRect(tVector(1, 0.5, -1, 0), tVector(0.2, 0.2, 0.2, 0));
    cDrawUtil::Translate(tVector(1, 0.5*mTime/10, mTime/100, 0));
//    cDrawUtil::Scale(tVector(mTime/2, 1, 1, 0));
	cDrawUtil::DrawCapsule(0.1,0.1);
