...
cCloth::tParams::tParams()
{
	...
	mTopPos2 = tVector(1, 1, 1, 0);
	mLength = 1;
	mRes = tVector(5, 5, 3, 0);
...
{
	return mParams.mRes[0] * mParams.mRes[1] * mParams.mRes[2];
}
...
int cCloth::GetVertID(int u, int v, int z) const
{
	assert(u >= 0 && u < mParams.mRes[0]);
	assert(v >= 0 && v < mParams.mRes[1]);
	assert(z >= 0 && v < mParams.mRes[2]);
	return z * mParams.mRes[0]* mParams.mRes[1] + v * mParams.mRes[0] + u;
}
	...
	for (int z = 0; z < res_z; ++z){
		for (int v = 0; v < res_v; ++v)
		{
			for (int u = 0; u < res_u; ++u)
			{
				...
				double z_lerp = z / (res_z - 1.0);
				pos = (1 - u_lerp) * mParams.mTopPos0 + u_lerp * mParams.mTopPos1;
				pos[1] = pos[1] - v_lerp * mParams.mLength;
				pos[2] = pos[2] - z_lerp * mParams.mLength;

				SetVertData(GetVertID(u, v, z), pos, mX);
			}
		}
	}
...
void cCloth::BuildEdges()
{
	...
	int res_z = mParams.mRes[2];
	//int z = 0;
	int num_edges = (res_u * (res_v - 1) + (res_u - 1) * res_u + 2 * (res_u - 1) * (res_v - 1))*res_z;
	mEdges.clear();
	...
	for (int z = 0; z < res_z; ++z){
		for (int v = 0; v < res_v; ++v)
		{
			...
			{
				tVector p0 = GetVertPos(GetVertID(u, v, z));
				tVector p1 = GetVertPos(GetVertID(u + 1, v, z));
				tEdge e;
				e.mVert0 = GetVertID(u, v, z);
				e.mVert1 = GetVertID(u + 1, v, z);
	...
	for (int z = 0; z < res_z; ++z) {
		...
		for (int v = 0; v < res_v - 1; ++v)
		{
			...
			{
				tVector p0 = GetVertPos(GetVertID(u, v, z));
				tVector p1 = GetVertPos(GetVertID(u, v + 1, z));
				tEdge e;
				e.mVert0 = GetVertID(u, v, z);
				e.mVert1 = GetVertID(u, v + 1, z);
				e.mRestLen = (p1 - p0).norm();
				AddEdge(e);
			}
		}
	...
	for (int z = 0; z < res_z-1; ++z) {
		for (int v = 0; v < res_v; ++v)
		{
			...
			{
				tVector p0 = GetVertPos(GetVertID(u, v, z));
				tVector p1 = GetVertPos(GetVertID(u, v, z+1));
				tEdge e;
				e.mVert0 = GetVertID(u, v, z);
				e.mVert1 = GetVertID(u, v, z+1);
				e.mRestLen = (p1 - p0).norm();
				AddEdge(e);
			}
		}
	...
	for (int z = 0; z < res_z; ++z) {
		...
		for (int v = 0; v < res_v - 1; ++v)
		{
			...
			{
				tVector p0 = GetVertPos(GetVertID(u, v, z));
				tVector p1 = GetVertPos(GetVertID(u + 1, v + 1, z));
				tEdge e0;
				e0.mVert0 = GetVertID(u, v, z);
				e0.mVert1 = GetVertID(u + 1, v + 1, z);
				e0.mRestLen = (p1 - p0).norm();
				AddEdge(e0);

				tVector p2 = GetVertPos(GetVertID(u + 1, v, z));
				tVector p3 = GetVertPos(GetVertID(u, v + 1, z));
				tEdge e1;
				e1.mVert0 = GetVertID(u + 1, v, z);
				e1.mVert1 = GetVertID(u, v + 1, z);
	...
	for (int z = 0; z < res_z-1; ++z) {
		...
		for (int v = 0; v < res_v - 1; ++v)
		{
			...
			{
				tVector p0 = GetVertPos(GetVertID(u, v, z));
				tVector p1 = GetVertPos(GetVertID(u, v + 1, z+1));
				tEdge e0;
				e0.mVert0 = GetVertID(u, v, z);
				e0.mVert1 = GetVertID(u, v + 1, z+1);
				e0.mRestLen = (p1 - p0).norm();
				AddEdge(e0);

				tVector p2 = GetVertPos(GetVertID(u, v, z+1));
				tVector p3 = GetVertPos(GetVertID(u, v + 1, z));
				tEdge e1;
				e1.mVert0 = GetVertID(u, v, z+1);
				e1.mVert1 = GetVertID(u, v + 1, z);
				e1.mRestLen = (p2 - p3).norm();
				AddEdge(e1);
			}
	...
	for (int z = 0; z < res_z-1; ++z) {
		...
		for (int v = 0; v < res_v; ++v)
		{
			for (int u = 0; u < res_u - 1; ++u)
			{
				tVector p0 = GetVertPos(GetVertID(u, v, z+1));
				tVector p1 = GetVertPos(GetVertID(u + 1, v, z));
				tEdge e0;
				e0.mVert0 = GetVertID(u, v, z);
				e0.mVert1 = GetVertID(u + 1, v, z+1);
				e0.mRestLen = (p1 - p0).norm();
				AddEdge(e0);

				tVector p2 = GetVertPos(GetVertID(u + 1, v, z));
				tVector p3 = GetVertPos(GetVertID(u, v, z+1));
				tEdge e1;
				e1.mVert0 = GetVertID(u + 1, v, z);
				e1.mVert1 = GetVertID(u, v, z+1);
				e1.mRestLen = (p2 - p3).norm();
				AddEdge(e1);
			}
		}
	...
	Eigen::VectorXd& out_X, Eigen::VectorXd& out_V)
{
	...
	Eigen::VectorXd out_dX(X.rows());
	Eigen::VectorXd out_dV(V.rows());
	...
	Eigen::VectorXd& out_X, Eigen::VectorXd& out_V)
{
	...
	out_X = X + timestep / 2 * out_dX;
	out_V = V + timestep / 2 * out_dV;

	EvalDerivative(out_X, out_V, out_dX, out_dV);
	...
	Eigen::VectorXd& out_X, Eigen::VectorXd& out_V)
{
	...
	Eigen::VectorXd out_dX(X.rows());
	Eigen::VectorXd out_dV(V.rows());
	Eigen::VectorXd dX(X.rows());
	Eigen::VectorXd dV(V.rows());

	EvalDerivative(X, V, out_dX, out_dV);

	dX = X + timestep*out_dX;
	dV = V + timestep*out_dV;         //Yi+hf(Yi)

	EvalDerivative(dX, dV, dX, dV);

	out_X = X + timestep / 2 * (out_dX + dX);
	out_V = V + timestep / 2 * (out_dV + dV);
}
	...
	Eigen::VectorXd& out_X, Eigen::VectorXd& out_V)
{
	...
	int r = mJ.rows();
	int c = mJ.cols();
	Eigen::SparseMatrix<double> I(r, c);
	I.setIdentity();
	I = I - timestep*mJ;
	...
	EvalDerivative(X, V, out_dX, out_dV);
	Eigen::VectorXd fY_i(X.rows() + V.rows());
	fY_i << out_dX, out_dV;
	fY_i = timestep*fY_i;

	mSolver.analyzePattern(I);
	mSolver.factorize(I);

	Eigen::VectorXd del_Y;
	del_Y = mSolver.solve(fY_i);

	out_X = X + del_Y.head(X.rows());
	out_V = V + del_Y.tail(V.rows());
